...
class RRRScrapper():
    ...
    def ScrapeNameData(driver):
        sel_name = '//div[contains(@class, "products__text__header")]/h3/a'
        return driver.find_elements(By.XPATH, sel_name)

    def ScrapeInfoData(driver):
        sel_name = '//div[contains(@class, "products__text__description__main")]/span'
        return driver.find_elements(By.XPATH, sel_name)

    def ScrapePartNrData(driver):
        sel_name = '//p[contains(@class, "products__code")]'
        part_nrs = []
        product_codes = driver.find_elements(By.XPATH, sel_name)
        for elem in product_codes:
            part_nrs.append(elem)
        return part_nrs
        ...
        with webdriver.Chrome(service=ChService('../drivers/chromedriver_v103.exe'), options=op) as driver:
            ...
            while continue_to_scrape:
                ...
                for j in range(0, len(parts_name)):
                    try:
                        clean_price = parts_prices[j].text
                        clean_price = clean_price.replace('€', '')
                        data_formated = [{"Part Name": parts_name[j].text, "Car Info": parts_info[j].text, "Part Code": parts_nr[j].text, "Price": clean_price}]
